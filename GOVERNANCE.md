...
Maintainers should use phishing-resistant or cryptographic second factors where available, such as passkeys, hardware security keys, or authenticator applications using time-based one-time passwords. SMS-only two-factor authentication is discouraged because it does not provide phishing-resistant protection.
